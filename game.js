            ...
            if (propina < 0) {
                ...
                if (!cambio) {
                    ...
                    if (propina > 0 && propina < 51) {
                        ...
                        print("Dejamos algo de propina bajo uno de los platos y salimos del lugar. [karma +1]\n");
                        karma += 1;
                        pasar1 = true;
                        return await path12();
                        ...
                        await sleep(b);
                        print("\nPor suerte recordamos salir con dinero en efectivo.");
                        print("Dejamos la propina correspondiente en la mesa y nos dirigimos a la salida. [karma +2]\n");
                        karma += 2;
                        pasar1 = true;
                        return await path12();
...
async function path11a() {
    ...
    await sleep(a);
    print("");
    await typeWriter('"Este café es como un indulto, che, algo terriblemente conciliatorio."', c);
...
async function path8() {
    ...
    if (parador === "parar" || parador === "p" || parador === "estacionar" || parador === "e") {
        await sleep(b);
...
async function path6() {
    ...
    if (charla === "planes" || charla === "p") {
        print("");
        await typeWriter('"Desde que regresé a Buenos Aires me costó adaptarme un poco...', c);
        await typeWriter('Encontrar dónde vivir, volver a conseguir trabajo, recuperar mis vínculos.', c);
        await typeWriter('Una vez que volví a acomodarme todo fue haciéndose más fácil...', c);
        ...
        await typeWriter('Hasta que de a poco fui haciendo pie y buscando más ayuda.', c);
        await typeWriter('Ahora tengo mis días. No te voy a decir que está todo espectacular,', c);
        await typeWriter('pero... ', c);
        await sleep(d);
        await typeWriter('estoy mucho mejor. Trato de rodearme de lo que me hace bien.', c);
        ...
        await sleep(d);
        await typeWriter('Cuando me invitaste estaba en uno de esos momentos...', c);
        ...
        await typeWriter('Me lo vendiste tan bien que en algún punto hasta pensé que había sido mi idea.', c);
...
async function pathJ() {
    ...
    if (["red hot chili peppers", "rhcp", "red hot", "chili peppers", "red hot chili pepers"].includes(musica)) {
        await sleep(b);
        print("\n♫ Suena 'Scar Tissue', de los Red Hot Chili Peppers. ♫\n");
        await typeWriter(`${companion}: "Altísima banda, no? Uh es un discazo este, un tema mejor que el otro."`, c);
        await typeWriter('        "Escuchá lo que es este solo de Frusciante, una locura."', c);
        musica = "los Red Hot Chili Peppers";
        await path4();
    } else {
        await sleep(b);
        await typeWriter(`${companion}: "Ah no querés escuchar los Red Hot...? Otra cosa? Uh perdón no traje nada, pensé que no íbamos a necesitar."`, c);
        await pathJ();
    }
}
...
async function path3() {
    ...
    if (["red hot chili peppers", "rhcp", "red hot", "chili peppers", "red hot chilli peppers", "red hot chili pepers"].includes(musica)) {
        await sleep(b);
        ...
        musica = "Taylor Swift";
        await path4();
    } else if (["queen", "freddie", "freddie mercury", "freddy"].includes(musica)) {
        await sleep(b);
        ...
        musica = "Bersuit";
        await path4();
    } else if (["redondos", "patricio rey", "los redondos", "indio", "indio solari"].includes(musica)) {
        await sleep(b);
        ...
        musica = "Wos";
        await path4();
    } else if (["rolling stones", "stones", "rolling", "los stones", "roling", "roling stones"].includes(musica)) {
        await sleep(b);
        print("\n♫ Suena 'Start Me Up.' ♫\nEl cuerpo comienza a moverse solo.\n");
        musica = "los Rolling Stones";
        await path4();
    } else if (["billie", "billie eilish", "skip bilie eilish", "bilie"].includes(musica)) {
        await sleep(b);
        print("\n♫ Suena 'Whish You Were Gay.' ♫\nEsa voz.\n");
        musica = "Billie Eilish";
        await path4();
    } else if (["cowboy bebop", "yoko kanno", "seatbelts", "the seatbelts"].includes(musica)) {
        await sleep(b);
        print("\n♫ Suena 'TANK.' ♫\nOK 3, 2, 1 LETS JAM\n");
        musica = "Cowboy Bebop";
        await path4();
    } else if (["evangelion", "opening evangelion", "neon genesis evangelion", "zankoku na tenshi", "zankoku"].includes(musica)) {
        await sleep(b);
        ...
        musica = "Daft Punk";
        await path4();
    } else if (["lo fi", "lo fi hip hop", "lofi", "lofi hip hop", "lofihiphop", "low fi", "lowfi", "lo fi radio", "lo fi hip hop radio", "lofi radio"].includes(musica)) {
        await sleep(b);
        ...
        musica = "Nombre";
        await path4();
    } else if (["musica", "house", "rock", "pop", "cumbia", "punk", "musica clasica", "clasica", "metal", "cuarteto"].includes(musica)) {
        await sleep(b);
        print("\nQué artista o banda?\n");
        await path3();
    } else {
...
async function path1() {
    print("Qué deseas hacer? No olvides nada! (nafta/cubiertas/aceite/vamos!/.../ayuda) ");
    let inputPath = await question();
    let secondPath = inputPath.toLowerCase().trim();

    if (["arrancar", "encender", "poner en marcha", "encender auto", "contacto", "marcha", "llave", "prender", "x"].includes(secondPath)) {
        if (arrancar) {
        ...
        await path1();
    } else if (["mapa", "direccion", "direcciones", "mapas"].includes(secondPath)) {
        if (mapa) {
            print("\nMapa preparado!\n");
        } else {
        ...
        await path1();
    } else if (["cambio", "monedas", "dinero", "plata", "efectivo", "billetes"].includes(secondPath)) {
        if (cambio) {
        ...
        await path1();
    } else if (["comida", "aliento", "alimentos", "sandwich", "sandwiches", "sandwichs", "pan", "galletas", "galletitas", "fruta", "frutas", "chocolate", "sandwiches de miga", "sandwichitos de miga", "almuerzo", "vianda", "bianda"].includes(secondPath)) {
        if (comida) {
        ...
        await path1();
    } else if (["bolsos", "bolso", "valija", "valijas", "maleta", "maletas", "equipaje"].includes(secondPath)) {
        if (bolsos) {
        ...
        await path1();
    } else if (["flores", "verde", "maria", "charuto", "thc", "marihuana", "hierva", "hierba", "fafafa", "fasito", "faso", "prensado", "mariguana", "droga", "drogas", "canabis", "cannabis", "porrito", "porro", "porros"].includes(secondPath)) {
        if (yerba2) {
...
async function inicioGame() {
    rta1 = "";
    while (rta1 !== "si" && rta1 !== "no" && rta1 !== "n" && rta1 !== "s" && rta1 !== "z") {
        if (repeticion) {
            await sleep(b);
    ...
    if (rta1[0] === "s") {
        ...
        print("Pasas por el lavadero y esperas tomando un café, mirando a la gente pasar por Avenida Rivadavia.");
        print("Pronto tendrás unos días libres, y has decidido pasarlos en la costa.\n");
        await sleep(b);
        print("Desde el comienzo sabías a quién le dirías que te acompañe. (Nombre) ");
